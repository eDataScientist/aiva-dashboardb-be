...
async def active_account(db_session):
    account = Account(
        email="analytics.routes@example.com",
        password_hash="hashed-password",
        full_name="Analytics Routes",
        role="analyst",
        is_active=True,
    )
...
@pytest.mark.asyncio
async def test_analytics_routes_happy_path_returns_expected_payload_shapes(
    client,
    db_session,
    active_account,
):
    ...
    summary = await client.get(
        f"/api/v1/analytics/summary?{common_params}",
    ...
    volume = await client.get(
        f"/api/v1/analytics/message-volume-trend?{common_params}",
        headers=_auth_headers(active_account),
    )
    ...
    peak_hours = await client.get(
        f"/api/v1/analytics/peak-hours?{common_params}",
        headers=_auth_headers(active_account),
    )
    ...
    quality = await client.get(
        f"/api/v1/analytics/quality-trend?{common_params}",
        headers=_auth_headers(active_account),
    )
    ...
    lead = await client.get(
        f"/api/v1/analytics/lead-conversion-trend?{common_params}",
        headers=_auth_headers(active_account),
    )
...
@pytest.mark.asyncio
async def test_analytics_routes_edge_cases_empty_db_are_zero_safe(client, active_account):
    params = "start_date=2026-02-01&end_date=2026-02-02&channel=all"

    summary = await client.get(
        f"/api/v1/analytics/summary?{params}",
        headers=_auth_headers(active_account),
    )
    assert summary.status_code == 200
    ...
    volume = await client.get(
        f"/api/v1/analytics/message-volume-trend?{params}",
        headers=_auth_headers(active_account),
    )
    assert volume.status_code == 200
    ...
    top_intents = await client.get(
        f"/api/v1/analytics/top-intents?{params}",
        headers=_auth_headers(active_account),
    )
    assert top_intents.status_code == 200
    ...
    peak_hours = await client.get(
        f"/api/v1/analytics/peak-hours?{params}",
        headers=_auth_headers(active_account),
    )
    assert peak_hours.status_code == 200
...
@pytest.mark.asyncio
async def test_analytics_routes_validation_errors(client, active_account):
    ...
    bad_channel = await client.get(
        "/api/v1/analytics/message-volume-trend?channel=sms",
        headers=_auth_headers(active_account),
    )
    assert bad_channel.status_code == 422

    bad_limit_low = await client.get(
        "/api/v1/analytics/top-intents?limit=0",
        headers=_auth_headers(active_account),
    )
    assert bad_limit_low.status_code == 422

    bad_limit_high = await client.get(
        "/api/v1/analytics/top-intents?limit=100",
        headers=_auth_headers(active_account),
    )
